...
func IndexView(db *sql.DB) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Select data order
		rows, err := db.Query("SELECT * FROM `pesanan` ORDER BY id DESC")
		if err != nil {
			w.Write([]byte(err.Error()))
			w.WriteHeader(http.StatusInternalServerError)
			return
		...
		orderList := []Order{}
		for rows.Next() {
			var id, idbarang, harga, jumlah int
			var foto, nama string

			err = rows.Scan(&id, &idbarang, &foto, &nama, &harga, &jumlah)
			if err != nil {
				w.Write([]byte(err.Error()))
				w.WriteHeader(http.StatusInternalServerError)
				return
